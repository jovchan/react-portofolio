...
export default function MyWorks() {
  ...
  return (
    <div>
      <div className="flex justify-between text-7xl mx-6 lg:text-9xl md:text-8xl sm:text-8xl text-abu druk">
        <p className={`transition-all duration-500 ${colorChange.c1 ? 'text-oren' : ''}`}>MY WORKS</p>
        <div className=" druk flex">
          <span className={`transition-all duration-500 ${colorChange.c2 ? 'text-oren' : ''}`}>1</span>
          <svg
          ...
          <span className={`transition-all duration-500 ${colorChange.c3 ? 'text-oren' : ''}`}>4</span>
        </div>
      </div>
